cap program drop permutation_naics2_out
program define permutation_naics2_out
syntax, filename(string) iterations(integer)

  foreach bin in 2 3 5 10 30 90 276 {
  	use "../output/naics_`bin'.dta", clear
    preserve
  	tempvar tv1 tv2 tv3
  	count
  	local total = r(N)
...
foreach bin in 2 3 5 10 30 90 276 {
 local filelist: dir "../output" files "naics_`bin'_*.dta"
 tempfile tf_collect
 ...
 save "../output/naics_`bin'.dta", replace
}
qui permutation_naics2_out, iterations(`iter') filename(../output/cdf_naics)
